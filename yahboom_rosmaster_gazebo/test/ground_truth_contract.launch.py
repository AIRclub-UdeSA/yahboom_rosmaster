...
def generate_test_description():
    ...
    probe = Node(
        ...
        executable="ground_truth_contract_probe.py",
        parameters=[{
            "timeout": 30.0,
            "samples": 10,
            "clock_tolerance": 0.1,
    ...
    return LaunchDescription([
        SetEnvironmentVariable(
            "IGN_PARTITION", f"yahboom_ground_truth_contract_{os.getpid()}"),
        ...
        # (~3.5 s in); the probe waits for the ground-truth topic on its own.
...
class TestGroundTruthContract(unittest.TestCase):
    """Require the ground-truth probe to finish successfully."""
